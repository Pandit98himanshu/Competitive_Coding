...
class PandaAndNumbers {
	...
	static Vector<Integer> pandaticNumbers = new Vector<>();
    static boolean[] isPossible = new boolean[MAX + 1];
	static void init(int n) {
	    // find all pandatic numbers
	    int pandaNum = 1;
	    for (int i = 1; pandaNum <= n; i++) {
	        pandaNum = (int) Math.pow(i, i);
	        pandaticNumbers.add(pandaNum);
	    }
	    // find whether we can reach pandatic numbers by a subtracting any digit
	    Arrays.fill(isPossible, false);
	    for (int i = 1; i <= n; i++) {
	        isPossible[i] = pandaticNumbers.contains(i) || solve(i);
	    }
	}
	...
	static boolean solve(int n) {
	    ...
	    while (temp > 0) {
	        ...
	        if (digit != 0) {
	            ...
	            ////
	            // THIS IS CATCH IN THIS QUESTION //
	            if (n > digitSq && isPossible[n - digitSq]) {
	                return true;
	            }
	        }
	        temp /= 10;
	...
	public static boolean solve1(int n) {
		...
		if (n > 0) {
		    for (int i = 0; i < num.length(); i++) {
                ...
                if (ele > 0) {          // infinite loop occurs when "ele" == 0
                    solve1(Integer.parseInt(num) - (ele * ele));
                }
            }
		}
