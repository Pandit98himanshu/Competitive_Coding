...
class PrimeNumbersAgain {

	static StringBuilder out = new StringBuilder();
	static long mod = (long) (1e9 + 7);
	static int MAX = (int) (1e4);
	static long[] fact = new long[MAX + 1];
	static boolean[] isPrimatic = new boolean[MAX + 1];
    static Vector<Integer> powpow = new Vector<>();
	static {
	    powpow.add(4);       // 2^2
	    powpow.add(27);      // 3^3
	    powpow.add(256);     // 4^4
	    powpow.add(3125);    // 5^5
	    powpow.add(46656);   // 6^6
	}
	
	public static void main(String[] args) throws IOException {
	
	    sieve(isPrimatic, MAX);
...
// https://www.geeksforgeeks.org/minimum-number-of-primes-required-such-that-their-sum-is-equal-to-n/
            ...
            if (isPrimatic[n]) {
                ...
                for (int i = 0; i < 5; i++) {
                    int val = n - powpow.get(i);
                    if (val > 1 && isPrimatic[val]) {
                        minCount = 2;
                        break;
                    }
                }
...
/*~~~~~~~~~~~~~~~~~    UTILITY METHODS    ~~~~~~~~~~~~~~~~~*/
	...
	static void sieve(boolean[] prime, int n) {
        ...
        for (int i = 2; i * i <= n; i++) {
            ...
            long primatic = (long) Math.pow(i, i);
            if (primatic <= n) {
                prime[(int) primatic] = true;
            }
        }
    }
